...
let __movableTabClickSuppressItem = null;
...
function clearMovableTabClickSuppression() {
    ...
    __movableTabClickSuppressItem = null;
}

function suppressClickAfterMovableTabDrag(event) {
    // Suppress only the synthetic click generated by releasing the same tab that
    // was just dragged. Clicks on any other tab must remain usable immediately.
    if (__movableTabClickSuppressItem) {
        if (!__movableTabClickSuppressItem.isConnected || !__movableTabClickSuppressItem.contains(event.target)) {
...
function suppressNextMovableTabClickBriefly(endX = 0, endY = 0, sourceItem = null) {
    clearMovableTabClickSuppression();
    __movableTabDragEndX = endX;
    __movableTabDragEndY = endY;
    __movableTabClickSuppressItem = sourceItem || null;
    document.addEventListener('click', suppressClickAfterMovableTabDrag, true);
    __movableTabClickSuppressTimer = window.setTimeout(clearMovableTabClickSuppression, 120);
}
